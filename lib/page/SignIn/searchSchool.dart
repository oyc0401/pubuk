...
class _SearchSchoolState extends State<SearchSchool> {
  ...
  Widget schoolButton(School school) {
    ...
    // int schoolCode = int.parse(school.schoolCode);
    return CupertinoButton(
      child: Text(schoolName),
      onPressed: () {
        school.level=3;
        school.cityCode=cityCode;
        saveSchoolCode(school);
      },
    );
  }
  ...
  void saveSchoolCode(School school) async {
    ...
    Navigator.of(context).pop(school.schoolToArray());
  }
}
    ...
    if (json['schulAflcoinfo'] == null) {
      ...
      for (Map element in MapList) {
        ...
        if (lastCode != schoolCode) {
          schools.add(School(name: schoolName, schoolCode: schoolCode));
          lastCode = schoolCode;
        }
      }
    }
...
class School {
  ...
  School({required this.name, required this.schoolCode});

  @override
  toString(){
    return "$name, $cityCode, $schoolCode, $level";
  }
  ...
  static listToSchool(List list){
    School school=School(name: list[0], schoolCode: list[2]);
    school.cityCode=list[1];
    school.level=list[3];
